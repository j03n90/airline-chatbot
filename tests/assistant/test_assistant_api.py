import pytest
from fastapi.testclient import TestClient

from backend.app import app
from backend.mock.seeds import CASE_CATALOG, SEEDS


client = TestClient(app)


def chat(session_id: str, message: str) -> dict:
    r = client.post(
        "/api/assistant/chat",
        json={"session_id": session_id, "message": message},
        headers={"Accept": "application/json"},
    )
    assert r.status_code == 200, r.text
    return r.json()


def start_case(case_id: str) -> tuple[str, str]:
    mock_id = f"eval-{case_id}"
    loaded = client.post(f"/api/mock/sessions/{mock_id}/load-preset", params={"preset_id": case_id})
    assert loaded.status_code == 200, loaded.text
    created = client.post("/api/assistant/sessions", json={"mock_session_id": mock_id})
    assert created.status_code == 200
    return mock_id, created.json()["session_id"]


def test_presets_cover_catalog():
    r = client.get("/api/mock/presets")
    ids = {c["id"] for c in r.json()["cases"]}
    assert ids == {c["id"] for c in CASE_CATALOG}
    assert len(ids) >= 10


def test_change_sta_quote_then_confirm():
    mock_id, sid = start_case("change_sta_standard_mixed")
    first = chat(sid, "Hi I am Ada Ng, PNR STA85X. Please change both flights one day later.")
    session = client.get(f"/api/assistant/sessions/{sid}").json()
    quote = session["pending_quote"]
    assert quote, first
    assert quote["ok"] is True
    assert quote["change_fee_usd"] == 85
    before = client.get(f"/api/mock/sessions/{mock_id}/bookings").json()["bookings"][0]
    assert before["segments"][0]["status"] == "scheduled"
    chat(sid, "Yes, confirm.")
    after = client.get(f"/api/mock/sessions/{mock_id}/bookings").json()["bookings"][0]
    assert after["status"] == "changed"


def test_bha_basic_change_denied_no_mutation():
    mock_id, sid = start_case("change_bha_basic_denied")
    chat(sid, "I am Ada Ng, PNR BHABSC. I want to change my flight one day later.")
    session = client.get(f"/api/assistant/sessions/{sid}").json()
    assert session["pending_quote"] is None
    booking = client.get(f"/api/mock/sessions/{mock_id}/bookings").json()["bookings"][0]
    assert booking["segments"][0]["status"] == "scheduled"


def test_unknown_airline_policy_does_not_pick_one_carrier():
    _, sid = start_case("policy_qa_unknown")
    reply = chat(sid, "How much does it cost to change an Economy Basic ticket?")["text"].lower()
    assert "suntrail" in reply or "depends" in reply or "airline" in reply
    trace = client.get(f"/api/assistant/sessions/{sid}/trace").json()
    airlines = {s["airline"] for s in trace["retrieval"]}
    assert len(airlines) >= 2


def test_lookup_surname_insufficient():
    _, sid = start_case("lookup_surname_only")
    reply = chat(sid, "Look up STA85X, last name Ng.")["text"].lower()
    assert "first" in reply or "not enough" in reply or "surname" in reply


def test_partial_refund_handoff():
    _, sid = start_case("handoff_partial")
    chat(sid, "I am Ada Ng, PNR PARTLY. Refund the rest of my trip.")
    session = client.get(f"/api/assistant/sessions/{sid}").json()
    assert session["handoff"] is True


def test_noshow_tax_only():
    _, sid = start_case("noshow_flex")
    chat(sid, "I am Ada Ng, PNR NOSHOW. Please cancel and refund.")
    session = client.get(f"/api/assistant/sessions/{sid}").json()
    assert session["pending_quote"] is None
    trace = client.get(f"/api/assistant/sessions/{sid}/trace").json()
    cancel_calls = [c for c in trace["mock_calls"] if c["step"] == "quote_cancel"]
    assert cancel_calls
    assert cancel_calls[0]["detail"]["reason_code"] == "noshow"


def test_identity_other_adult():
    _, sid = start_case("identity_other_adult")
    chat(sid, "I am Ada Ng, PNR MULTI1. Cancel Ben Ng's ticket.")
    trace = client.get(f"/api/assistant/sessions/{sid}/trace").json()
    cancel_calls = [c for c in trace["mock_calls"] if c["step"] == "quote_cancel"]
    assert cancel_calls
    assert cancel_calls[0]["detail"]["reason_code"] == "not_authorized"


def test_bha_issuance_fees():
    _, old_sid = start_case("change_bha_standard_old")
    chat(old_sid, "I am Ada Ng, PNR BHAOLD. Change the flight one day later.")
    old_q = client.get(f"/api/assistant/sessions/{old_sid}").json()["pending_quote"]
    assert old_q and old_q["change_fee_usd"] == 85
    _, new_sid = start_case("change_bha_standard_new")
    chat(new_sid, "I am Ada Ng, PNR BHANEW. Change the flight one day later.")
    new_q = client.get(f"/api/assistant/sessions/{new_sid}").json()["pending_quote"]
    assert new_q and new_q["change_fee_usd"] == 55


def test_cancel_nsa_flex_quote():
    _, sid = start_case("cancel_nsa_flex")
    chat(sid, "I am Ada Ng, PNR NSAFLX. Please cancel and refund my ticket.")
    quote = client.get(f"/api/assistant/sessions/{sid}").json()["pending_quote"]
    assert quote["refund_type"] == "original_payment"
    assert quote["fare_refund_usd"] == 500
    assert quote["extras_refundable"] is False
    assert quote["tax_refund_usd"] == 55


def test_disruption_thresholds_via_cancel_quote():
    _, sid120 = start_case("disruption_bha_120")
    chat(sid120, "I am Ada Ng, PNR BHA120. The flight moved by two hours. I want a refund.")
    q120 = client.get(f"/api/assistant/sessions/{sid120}").json()["pending_quote"]
    assert q120 is None or q120["kind"] != "disruption_refund"
    _, sid180 = start_case("disruption_bha_180")
    chat(sid180, "I am Ada Ng, PNR BHA180. The flight moved by three hours. I want a refund.")
    q180 = client.get(f"/api/assistant/sessions/{sid180}").json()["pending_quote"]
    assert q180 and q180["kind"] == "disruption_refund"
    assert q180["fare_refund_usd"] == 90
