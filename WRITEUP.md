...
`uv run pytest` — 62 tests covering the fee matrix, mock state, retrieval isolation, routing, and assistant conversations (success, denial, handoff, no-show, issuance-date fees). See [eval/cases.md](eval/cases.md). One full change+confirm trace is in [eval/trace-one-conversation.md](eval/trace-one-conversation.md).
...
- Multi-user concurrency is in-process memory, one process.
